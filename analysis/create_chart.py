...
from config.conclusion_table import conclusion_t1, conclusion_t2, conclusion_t18, conclusion_t14, conclusion_t15, \
    conclusion_t13, conclusion_t12, conclusion_t8, conclusion_t4, conclusion_t16, conclusion_t5, conclusion_t3, \
    conclusion_t10, conclusion_t11, conclusion_t6, conclusion_t7, conclusion_t19, conclusion_t20
from config.utils import conclution_high_salary_technology, technology_list, \
    change_dict, commany_list, degree_dict
...
def t1(pa):
    ...
    y = conclusion_t1()
    box.add('传统职业', ['薪水'], [y[1]], is_toolbox_show=False)
    box.add('新兴职业', ['薪水'], [y[0]], is_toolbox_show=False)
...
def t2(pa):
    ...
    x, y = [], []
    for i in conclusion_t2():
        x.append(i[0])
        y.append(i[1])

    bar.add('需求', x, y, is_toolbox_show=False)
    return bar
...
def t3(pa):
    ...
    x, y, z = conclusion_t3()
    hm.add("需求量", x, y, z, is_visualmap=True, visual_range=[1, 500],
...
def t4(pa):
    y, y1 = [], []
    for i in conclusion_t4():
        y.append(list(i.keys())[0])
        y1.append(list(i.values())[0])

    bar = p.Bar("薪资前10城市")
    bar.add("薪资", y, y1, mark_line=["average"], is_toolbox_show=False)
    return bar
...
def t7(pa):
    ...
    x, y, z = change_dict(conclusion_t7())
    scatter.add("薪水", x, y, z, is_visualmap=True,
...
def t8(pa):
    y, y1 = [], []
    for i in conclusion_t8():
        y.append(i[0])
        y1.append(i[1])

    bar = p.Bar("山东薪水前10的城市排名")
    bar.add("薪水", y, y1, mark_line=['average'], is_toolbox_show=False)
    return bar
...
def t9(pa):
    ...
    chart = p.Geo("山东省计算机职位分布", '数据来自齐鲁人才网，部分地区数据不准确', **style.init_style,
...
def t11(pa):
    bar = p.Bar('经验要求前十的职位')
    x, y = change_dict(conclusion_t11())
    bar.add('经验', x, y, mark_line=["average"], is_toolbox_show=False)
    return bar
...
def t12(pa):
    ...
    pa = conclusion_t12()
    scatter3D.add("3D", pa, is_visualmap=True, visual_range_color=range_color, is_grid3d_rotate=True,
                  visual_range=[7000, 35000]
                  , xaxis3d_name='经验', yaxis3d_name='需求', zaxis3d_name='薪水', is_toolbox_show=False,
                  visual_top=9999)
    return scatter3D
...
def t15(pa):
    x, y = [], []
    for i in conclusion_t15():
        x.append(i[0])
        y.append(i[1])

    bar = p.Bar('薪水前十的职位')
    bar.add('薪水', x, y, mark_line=["average"], is_toolbox_show=False)
    return bar
...
def t17(pa):
    ...
    for i in conclution_high_salary_technology(technology_list, 10, 1):
        y.append(i[0])
        y_values.append(i[1])
...
def t18(pa):
    ...
    x, y = [], []
    for i in conclusion_t18().items():
        x.append(i[0])
        y.append(i[1])

    liquid.add(x, y, is_liquid_outline_show=False, shape=shape, is_toolbox_show=False)
    return liquid


@ways
def t19(pa):
    l1 = copy.deepcopy(commany_list)
    pp = list(degree_dict.keys())
    data = conclusion_t19()
...
def t20(pa):
    ...
    x = conclusion_t20()
    scatter.add('工资', commany_list, x[0], extra_data=x[1], is_visualmap=True, visual_dimension=2,
